...
public class CustomerRestExceptionHandler {
	...
	public ResponseEntity<CustomerErrorResponse> handleException(CustomerNotFoundException exc) {
		...
		return new ResponseEntity<>(error, HttpStatus.NOT_FOUND); // przekazanie w odpowiedzi jako body wyjątku
	}
	...
	public ResponseEntity<CustomerErrorResponse> handleException(Exception exc) {
		...
		CustomerErrorResponse error = new CustomerErrorResponse(HttpStatus.BAD_REQUEST.value(), exc.getMessage(),
				System.currentTimeMillis());
		// Return ResponseEntity
		return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST); // przekazanie w odpowiedzi jako body wyjątku
	}
}
